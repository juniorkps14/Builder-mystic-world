  ...
  const presetPositions = [
    {
      name: "Home Position",
      icon: Home,
      joints: { base: 0, shoulder: 0, elbow: 0, wrist1: 0, wrist2: 0, wrist3: 0 },
      description: "Safe home position",
    },
    {
      name: "Pick Position",
      icon: Hand,
      joints: { base: 45, shoulder: -30, elbow: 90, wrist1: -60, wrist2: 0, wrist3: 0 },
      description: "Optimal picking position",
    },
    {
      name: "Place Position",
      icon: Target,
      joints: { base: -45, shoulder: -45, elbow: 120, wrist1: -75, wrist2: 0, wrist3: 0 },
      description: "Standard placing position",
    },
    {
      name: "Inspection",
      icon: Activity,
      joints: { base: 0, shoulder: -90, elbow: 90, wrist1: 0, wrist2: 90, wrist3: 0 },
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <p className="text-slate-300 font-light">
                6-DOF robotic arm control with precision positioning and force feedback
              </p>
            </div>
            
            <div className="flex items-center gap-4">
              <Badge className={`bg-gradient-to-r ${getStatusColor(getCurrentStatus())} text-white border-0 px-4 py-2`}>
                {getCurrentStatus().charAt(0).toUpperCase() + getCurrentStatus().slice(1)}
              </Badge>
              <Button
                onClick={() => setArmStatus(prev => ({ ...prev, power: !prev.power }))}
                className={`${
                  armStatus.power 
        ...
        <Card className="bg-white/10 backdrop-blur-xl border border-white/20 p-6 shadow-xl hover:bg-white/15 transition-all duration-300">
          <div className="flex items-center justify-between">
            ...
            <div className={`h-12 w-12 rounded-xl flex items-center justify-center ${
              ...
              <Shield className={`h-6 w-6 ${armStatus.error ? "text-red-400" : "text-emerald-400"}`} />
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-white/10 backdrop-blur-xl border border-white/20 p-6 shadow-xl hover:bg-white/15 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-light text-white mt-1 capitalize">{armStatus.mode}</p>
            </div>
        ...
        <Card className="bg-white/10 backdrop-blur-xl border border-white/20 p-6 shadow-xl hover:bg-white/15 transition-all duration-300">
          <div className="flex items-center justify-between">
            ...
            <div className={`h-12 w-12 rounded-xl flex items-center justify-center ${
              ...
              <CheckCircle className={`h-6 w-6 ${armStatus.homed ? "text-emerald-400" : "text-yellow-400"}`} />
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="p-6">
              <Tabs defaultValue="joints" className="w-full">
                <TabsList className="grid w-full grid-cols-4 bg-white/10 border border-white/20">
                  <TabsTrigger value="joints" className="data-[state=active]:bg-white/20">
                    Joint Control
                  </TabsTrigger>
                  <TabsTrigger value="cartesian" className="data-[state=active]:bg-white/20">
                    Cartesian
                  </TabsTrigger>
                  <TabsTrigger value="gripper" className="data-[state=active]:bg-white/20">
                    Gripper
                  </TabsTrigger>
                  <TabsTrigger value="teach" className="data-[state=active]:bg-white/20">
                    Teach Mode
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="joints" className="mt-6">
                  <div className="space-y-6">
                    <h3 className="text-lg font-light text-white">Joint Position Control</h3>
                    
                    {Object.entries(joints).map(([jointName, value]) => (
                      <div key={jointName} className="bg-white/5 rounded-xl p-4 border border-white/10">
                        <div className="flex items-center justify-between mb-3">
                          <label className="text-white font-medium capitalize">{jointName}</label>
                        ...
                        <Slider
                          value={value}
                          onValueChange={(newValue) => updateJoint(jointName, newValue)}
                          max={180}
                          min={-180}
                          step={1}
                          className="w-full"
                ...
                <TabsContent value="cartesian" className="mt-6">
                  <div className="space-y-6">
                    <h3 className="text-lg font-light text-white">Cartesian Position Control</h3>
                    
                    {/* Position Control */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {Object.entries(endEffector).slice(0, 3).map(([axis, value]) => (
                        <div key={axis} className="bg-white/5 rounded-xl p-4 border border-white/10">
                          <div className="flex items-center justify-between mb-3">
                            <label className="text-white font-medium uppercase">{axis}</label>
                            <Badge className="bg-white/20 text-white border border-white/30">
                          ...
                          <Slider
                            value={value}
                            onValueChange={(newValue) => updateEndEffector(axis, newValue)}
                            max={axis === "z" ? 600 : 400}
                            min={axis === "z" ? 50 : -400}
                            step={1}
                            className="w-full"
                    ...
                    {/* Orientation Control */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {Object.entries(endEffector).slice(3, 6).map(([axis, value]) => (
                        <div key={axis} className="bg-white/5 rounded-xl p-4 border border-white/10">
                          <div className="flex items-center justify-between mb-3">
                            <label className="text-white font-medium capitalize">{axis}</label>
                            <Badge className="bg-white/20 text-white border border-white/30">
                              {value[0].toFixed(1)}°
                            </Badge>
                          </div>
                ...
                <TabsContent value="gripper" className="mt-6">
                  <div className="space-y-6">
                    <h3 className="text-lg font-light text-white">Gripper Control</h3>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                        <div className="flex items-center justify-between mb-3">
                          <label className="text-white font-medium">Gripper Position</label>
                          <Badge className="bg-white/20 text-white border border-white/30">
                            {gripper.position[0]}%
                          </Badge>
                        </div>
                        <Slider
                          ...
                          onValueChange={(value) => setGripper(prev => ({ ...prev, position: value }))}
                          max={100}
                          min={0}
                          step={1}
                          className="w-full"
                      ...
                      <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                        <div className="flex items-center justify-between mb-3">
                          <label className="text-white font-medium">Gripping Force</label>
                          <Badge className="bg-white/20 text-white border border-white/30">
                            {gripper.force[0]}%
                          </Badge>
                        </div>
                        <Slider
                          value={gripper.force}
                          onValueChange={(value) => setGripper(prev => ({ ...prev, force: value }))}
                          max={100}
                          min={0}
                          step={1}
                          className="w-full"
                ...
                <TabsContent value="teach" className="mt-6">
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-xl font-light text-white mb-2">Teach Mode</h3>
                    <p className="text-slate-300 mb-6">Move the arm manually to record positions</p>
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-white/10 backdrop-blur-xl border border-white/20 shadow-xl">
            <div className="p-6">
              <h3 className="text-lg font-light text-white mb-4">Preset Positions</h3>
            ...
            <Card className="bg-blue-500/20 border border-blue-500/30 shadow-xl">
              <div className="p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-lg font-light text-blue-300">Moving to Position</h3>
                </div>
                <Progress value={65} className="h-2 bg-blue-900/50" />
                <p className="text-blue-200 text-sm mt-2">Estimated time: 1.5s remaining</p>
              </div>
            </Card>
          )}
        </div>
